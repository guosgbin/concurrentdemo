        ...
        t1 = new Thread() {
            ...
            public void run() {
                for (int i = 0; i < (loopFlag); i++) {
                    Object a = new Object();
                    list.add(a);
                    System.out.println("加锁前" + (i + 1) + " " + ClassLayout.parseInstance(a).toPrintable());
                    synchronized (a) {
                        System.out.println("加锁中" + (i + 1) + " " + ClassLayout.parseInstance(a).toPrintable());
                    }
                    System.out.println("加锁结束" + (i + 1) + " " + ClassLayout.parseInstance(a).toPrintable());
                }
